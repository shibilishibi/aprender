<?php
		...
		$row_teahcer = mysql_fetch_array($query_teacher);
			/* teacher */
		$query_teache = mysql_query("SELECT * FROM users WHERE username='$username' AND password='$password'")or die(mysql_error());
		$num_row_teache = mysql_num_rows($query_teache);
		$row_teahce = mysql_fetch_array($query_teache);
		if( $num_row > 0 ) { 
